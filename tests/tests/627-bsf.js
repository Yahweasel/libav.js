...
// Null bitstream filter using our meta API
...
const outPackets = await libav.ff_bsf_multi(bsf, pkt, packets[streamIdx], true);
if (outPackets.length !== packets[streamIdx].length)
    throw new Error("Packets not passed thru null BSF");
await libav.av_bsf_free_js(bsf);

// Decode them
const frames = await libav.ff_decode_multi(c, pkt, frame, outPackets, true);
