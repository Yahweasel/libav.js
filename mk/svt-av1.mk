...
SVT_AV1_VERSION=1.8.0-59645eea34e2815b627b8293aa3af254eddd0d69
SVT_AV1_TAR_VERSION=2.1.2
...
# Use: buildrule(target name, extra deps, extra cmake flags, cflags)


# Non-threaded

build/SVT-AV1-v$(SVT_AV1_VERSION)/build-base/Makefile: build/inst/base/lib/libemfiberthreads.a build/SVT-AV1-v$(SVT_AV1_VERSION)/PATCHED | build/inst/base/cflags.txt
	...
	cd build/SVT-AV1-v$(SVT_AV1_VERSION)/build-base && \
		...
		-DCMAKE_C_FLAGS="-Oz `cat $(PWD)/build/inst/base/cflags.txt` -I$(PWD)/build/inst/base/include -L$(PWD)/build/inst/base/lib" \
		-DCMAKE_CXX_FLAGS="-Oz `cat $(PWD)/build/inst/base/cflags.txt` -I$(PWD)/build/inst/base/include -L$(PWD)/build/inst/base/lib" \
...
build/SVT-AV1-v$(SVT_AV1_VERSION)/CMakeLists.txt: build/SVT-AV1-v$(SVT_AV1_TAR_VERSION).tar.bz2
	cd build && tar jxf SVT-AV1-v$(SVT_AV1_TAR_VERSION).tar.bz2
	touch $@

build/SVT-AV1-v$(SVT_AV1_TAR_VERSION).tar.bz2:
	mkdir -p build
	curl https://gitlab.com/AOMediaCodec/SVT-AV1/-/archive/v1.8.0/SVT-AV1-v$(SVT_AV1_VERSION).tar.bz2 -L -o $@

SVT-AV1-release:
	cp build/SVT-AV1-v$(SVT_AV1_TAR_VERSION).tar.bz2 dist/release/libav.js-$(LIBAVJS_VERSION)/sources/
